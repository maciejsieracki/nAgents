...
| Proxy | `[REDACTED]:35001`, CA `/root/.ccr/ca-bundle.crt` | odmowy 403 zgłaszać, nie obchodzić |
...
## 2A. Aktualna dyrektywa właściciela — web-first

NAgents jest głównym projektem. AutoBot Router, AutoBot Monitor, integracje z
Hermesem i przyszła nakładka Desktopu są jego elementami.

Kolejność dostarczenia:

1. Najpierw znaleźć i potwierdzić bezpieczny, prosty dostęp przez serwerową
   wersję webową Hermesa albo webową warstwę NAgents. Pracownik ma dostać
   gotowy profil i czat bez konfigurowania gatewaya, serwera, modeli,
   poświadczeń i routingu.
2. Dopiero potem przygotować nakładkę na Desktop Hermesa albo dostosować jego
   powierzchnię. Desktop jest klientem dodatkowym, nie warunkiem życia procesu.

Cała hierarchia i użyteczność agentów ma być dostępna w webie. Ustawienia
zaawansowane są tylko dla administratora przez web administracyjny
NAgents/Hermesa albo terminal. Serwer jest właścicielem sesji, kolejki,
workerów, pamięci i audytu; zamknięcie klienta nie może zatrzymywać pracy.

Scenariusze tej dyrektywy są zapisane jako U1–U6 w `docs/spec/scenarios.md`, a
decyzja jako D-012 w `docs/spec/decisions.md`.

## 2B. Aktualna decyzja właściciela — serwerowy pomocnik procesu

Właściciel wybrał wariant A: niezależny od Desktopu serwerowy pomocnik z trwałą
instrukcją procesu. Decyzja jest zapisana jako `D-013`, a wybór jako
`NAG-INFRA-002-pomocnik-serwerowy-Q1 = A` w `docs/process/echo.md`.

Pomocnik ma odbierać dyspozycje serwerowego Crona i prowadzić wyłącznie
kwalifikowane przejścia istniejącego grafu:
...
Rutynowe przejścia mogą być wykonywane bez obecności właściciela. `TRIAGE`,
`TODO`, `BLOCKED`, braki routingu, brak dowodu, pytania produktowe oraz każda
niejednoznaczność pozostają zatrzymane i są eskalowane. Pomocnik nie tworzy
nowego zakresu, nie zmienia decyzji, nie integruje, nie pushuje, nie scala i nie
wdraża.

Właściciel polecił prowadzić pętlę AutoBot autonomicznie podczas swojej
nieobecności, aż kwalifikowane tematy dojdą do terminalnych bramek. Oznacza to
uruchamianie następnych legalnych faz, nie obchodzenie zależności ani bramek
właścicielskich. Scenariusze U7–U11 opisują wymagany test ciągłości po zamknięciu
Desktopu.
